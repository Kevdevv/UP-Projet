...
async function userCreate(req, res) {
    ...
    try {
        ...
        const User = req.app.get("models").User
        const NewUser = await new User({firstName : req.body.firstName, lastName : req.body.lastName, dateOfBirth : req.body.dateOfBirth, token, salt, hash,}).save()
        res.json(NewUser)
    }catch (error) {
...
async function userUpdate(req, res) {
    ...
    try {
        if (!req.body._id || !req.body.toModify) {
           return res.json("_id ou champs manquant")
        }
